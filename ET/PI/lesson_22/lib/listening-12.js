...
const buildMatchingSlide = (data = {}, context = {}) => {
  ...
  const isLongTextMatching = normalizeValue(data?.type) === "long_text";
  ...
  if (isLongTextMatching) {
    layout.classList.add("listening-word-match--long-text");
    slide.classList.add("listening-slide--matching-long-text");
  }
  ...
  sentencesColumn.className = isLongTextMatching
    ? "word-match-lines"
    : "word-match-sentences";
  ...
  const createSentenceCard = (entry, index) => {
    ...
    card.className = isLongTextMatching
      ? "word-match-line"
      : "word-match-sentence";

    const title = document.createElement("h3");
    title.textContent = isLongTextMatching ? entry.itemB : `${index + 1}`;
    ...
    placeholder.textContent = isLongTextMatching
      ? "Drop the matching sentence here"
  ...
  const createCard = (entry) => {
    ...
    if (isLongTextMatching) {
      card.classList.add("word-match-card--long-text");
    }
...
export const buildListeningTwelveSlides = (activityData = {}, context = {}) => {
