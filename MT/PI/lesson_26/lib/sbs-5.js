...
const normalizeMatchingKeywords = (keywordEntries = [], fallbackDialogues = []) => {
  const normalizedFromKeywords = (Array.isArray(keywordEntries) ? keywordEntries : [])
    .map((entry, index) => {
      const keyword = trimString(
        entry?.word_1 ?? entry?.word1 ?? entry?.word ?? entry?.label ?? entry?.text
      );
      const cardText = trimString(
        entry?.word_2 ?? entry?.word2 ?? entry?.definition ?? entry?.meaning
      );
      const normalizedKeyword = normalizeKeyword(keyword);
      const image = trimString(entry?.image ?? entry?.img ?? entry?.picture);
      if (!keyword || !normalizedKeyword || (!image && !cardText)) {
        return null;
      }
      return {
...
const buildPreListeningSlide = (
  ...
  const hasWordMatches = keywordItems.some((item) => item.matchType === 'word');

  const slide = document.createElement('section');
  slide.className = 'slide slide--pre-listening';
  if (hasWordMatches) {
    slide.classList.add('slide--pre-listening-word-match');
  }
  slide.innerHTML = `
    <h2>${activityLabel}${subActivitySuffix}</h2>
    <p class="slide__instruction">${
      hasWordMatches
        ? 'Drag each word to the correct meaning.'
  ...
  shuffledKeywords.forEach((item) => {
    const { keyword, normalizedKeyword, image, cardText, matchType } = item;
    ...
    if (matchType === 'word') {
      card.classList.add('pre-listening-card--word');
    }
    card.dataset.keyword = normalizedKeyword;

    if (matchType === 'word') {
      const text = document.createElement('p');
      text.className = 'pre-listening-card__text';
    ...
    card.dataset.label = matchType === 'word' ? cardText : keyword;

    gallery.appendChild(card);
    cardItems.push({ keyword, normalizedKeyword, matchType });
  });
      ...
      drop(event, ui) {
        ...
        if (expected === actual) {
          ...
          if (allComplete) {
            showCompletionModal({
              title: 'Great Work!',
              message: hasWordMatches
                ? 'You matched all of the words correctly.'
                : 'You matched all of the pictures correctly.',
            });
          }
        } else {
...
const buildSpeakingSlide = (
  ...
  const resetCards = () => {
    cards.forEach(({ card, prompt, entries, hasExplicitSpeakingEntries }) => {
      card.classList.remove('is-active');
      if (prompt) {
        prompt.hidden = true;
      }
  ...
  const runSpeakingPractice = async () => {
    ...
    try {
      for (let index = 0; index < cards.length; index += 1) {
        ...
        const { card, prompt, pairs, entries, hasExplicitSpeakingEntries } = item;
...
export const buildSbsFiveSlides = (activityData = {}, context = {}) => {
