...
const MATCHING_INSTRUCTION_TEXT =
  "Press Start to play. Match each keyword with its picture.";
...
const buildMatchingSlideId = (activityNumber, letter = "") => {
  ...
  if (activityNumber) {
    return `activity-${activityNumber}${suffix}-game4`;
  }
  return `activity${suffix}-game4`;
};
...
const extractInstructionEntry = (instructions, keys = []) => {
  ...
  for (const key of keys) {
    ...
    if (typeof value === "object") {
      const text = trimText(value.text);
      const audio = trimText(value.audio);
      if (text || audio) {
        return { text, audio };
      }
    }
...
const applyCustomInstruction = (slideObj, instructionEntry) => {
  if (!slideObj?.element || !instructionEntry) {
    return slideObj;
  }

  const instructionEl = slideObj.element.querySelector(".slide__instruction");
  if (instructionEl && instructionEntry.text) {
    instructionEl.textContent = instructionEntry.text;
  }

  if (instructionEntry.audio) {
    slideObj.instructionAudio = instructionEntry.audio;
  }

  slideObj.hasCustomInstructions = true;
  return slideObj;
};
...
export const buildInteractive9Slides = (activityData = {}, context = {}) => {
  ...
  const matchingA = normalizeMatchingPairs(activityData?.content?.activity_a?.content);
  if (matchingA.length) {
    const slide = createMatchingSlide(activityData.content.activity_a, {
      slideId: buildMatchingSlideId(activityNumber, "a"),
    ...
    slides.push(
      applyCustomInstruction(
        slide,
        extractInstructionEntry(instructions, ["activity_a"])
      )
    );
  }

  const matchingB = normalizeMatchingPairs(activityData?.content?.activity_b?.content);
  if (matchingB.length) {
    const slide = createMatchingSlide(activityData.content.activity_b, {
      slideId: buildMatchingSlideId(activityNumber, "b"),
      activityLabel: formatActivityLabel(activityNumber, "b"),
      focusText,
      includeFocus: shouldIncludeFocus(),
    });
    slides.push(
      applyCustomInstruction(
        slide,
        extractInstructionEntry(instructions, ["activity_b"])
      )
    );
  }

  const listenRepeatItems = normalizeListenRepeatItems(activityData?.content?.activity_c);
  if (listenRepeatItems.length) {
    const slide = createListenRepeatSlide(listenRepeatItems, {
      slideId: buildListenRepeatSlideId(activityNumber, "c"),
      activityLabel: formatActivityLabel(activityNumber, "c"),
      focusText,
      includeFocus: shouldIncludeFocus(),
    ...
    slides.push(
      applyCustomInstruction(
        slide,
        extractInstructionEntry(instructions, ["activity_c", "activity_bc"])
      )
    );
  }
  ...
  const fallbackSlide = createMatchingSlide(
    { content: [] },
    {
