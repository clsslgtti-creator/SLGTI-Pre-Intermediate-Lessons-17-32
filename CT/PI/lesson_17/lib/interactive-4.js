import {
  ...
  normalizeFirstSentence,
} from "./games/game-2.js";
import { audioManager, computeSegmentGapMs } from "./audio-manager.js";

const GAME_INSTRUCTION_TEXT =
  "Press Start to play. Arrange the jumbled words to form the sentence before time runs out.";
...
const buildGameSlideId = (activityNumber, letter = "") => {
  ...
  if (activityNumber) {
    return `activity-${activityNumber}${suffix}-game2`;
  }
  return `activity${suffix}-game2`;
};
...
const createGameSlide = (gameConfig = {}, context = {}) => {
  ...
  const stageId = `game2-stage-${Math.random().toString(36).slice(2, 8)}`;
  stage.id = stageId;
  ...
  const questions = normalizeWordEntries(
    gameConfig?.questions ?? gameConfig?.content
  );
  const firstSentence = normalizeFirstSentence(
    gameConfig?.firstSentence ??
      gameConfig?.first_sentence ??
      gameConfig?.first_sentance
  );
...
const collectGameActivities = (activityData = {}) => {
  ...
  const defaultFirstSentence =
    activityData?.first_sentence ?? activityData?.first_sentance ?? null;
  ...
  const buildEntry = (raw) => {
    ...
    const questions = Array.isArray(raw.content) ? raw.content : [];
    if (!questions.length) {
      return null;
    }
    return {
...
export const buildInteractive4Slides = (activityData = {}, context = {}) => {
